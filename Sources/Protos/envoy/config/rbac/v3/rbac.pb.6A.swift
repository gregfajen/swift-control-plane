...
///       principals:
///         - any: true
public struct Envoy_Config_Rbac_V3_RBAC {
  ...
  public var action: Envoy_Config_Rbac_V3_RBAC.Action = .allow

  /// Maps from policy name to policy. A match occurs when at least one policy matches the request.
  public var policies: Dictionary<String,Envoy_Config_Rbac_V3_Policy> = [:]
...
extension Envoy_Config_Rbac_V3_RBAC.Action: CaseIterable {
  // The compiler won't synthesize support with the UNRECOGNIZED case.
  public static var allCases: [Envoy_Config_Rbac_V3_RBAC.Action] = [
    .allow,
    .deny,
  ]
}
...
public struct Envoy_Config_Rbac_V3_Policy {
  // SwiftProtobuf.Message conformance is added in an extension below. See the
  // `Message` and `Message+*Additions` files in the SwiftProtobuf library for
  // methods supported on all messages.
  ...
  public var permissions: [Envoy_Config_Rbac_V3_Permission] = []

  /// Required. The set of principals that are assigned/denied the role based on “action”. Each
  /// principal is matched with OR semantics. To match all downstreams for this policy, a single
  /// Principal with the `any` field set to true should be used.
  public var principals: [Envoy_Config_Rbac_V3_Principal] = []
...
/// [#next-free-field: 11]
public struct Envoy_Config_Rbac_V3_Permission {
  // SwiftProtobuf.Message conformance is added in an extension below. See the
  // `Message` and `Message+*Additions` files in the SwiftProtobuf library for
  // methods supported on all messages.
  ...
  public var andRules: Envoy_Config_Rbac_V3_Permission.Set {
    get {
      if case .andRules(let v)? = _storage._rule {return v}
      return Envoy_Config_Rbac_V3_Permission.Set()
    }
    set {_uniqueStorage()._rule = .andRules(newValue)}
  }

  /// A set of rules where at least one must match in order to define the action.
  public var orRules: Envoy_Config_Rbac_V3_Permission.Set {
    get {
  ...
  public var urlPath: Envoy_Type_Matcher_V3_PathMatcher {
    get {
  ...
  public var destinationIp: Envoy_Config_Core_V3_CidrRange {
    get {
  ...
  public var metadata: Envoy_Type_Matcher_V3_MetadataMatcher {
    get {
  ...
  public var notRule: Envoy_Config_Rbac_V3_Permission {
    get {
  ...
  /// setup SNI.
  public var requestedServerName: Envoy_Type_Matcher_V3_StringMatcher {
    get {
  ...
  public enum OneOf_Rule: Equatable {
    /// A set of rules that all must match in order to define the action.
    case andRules(Envoy_Config_Rbac_V3_Permission.Set)
    /// A set of rules where at least one must match in order to define the action.
    case orRules(Envoy_Config_Rbac_V3_Permission.Set)
    /// When any is set, it matches any action.
    case any(Bool)
    /// A header (or pseudo-header such as :path or :method) on the incoming HTTP request. Only
    /// available for HTTP request.
    /// Note: the pseudo-header :path includes the query and fragment string. Use the `url_path`
    /// field if you want to match the URL path without the query and fragment string.
    case header(Envoy_Config_Route_V3_HeaderMatcher)
    /// A URL path on the incoming HTTP request. Only available for HTTP.
    case urlPath(Envoy_Type_Matcher_V3_PathMatcher)
    /// A CIDR block that describes the destination IP.
    case destinationIp(Envoy_Config_Core_V3_CidrRange)
    /// A port number that describes the destination port connecting to.
    case destinationPort(UInt32)
    /// Metadata that describes additional information about the action.
    case metadata(Envoy_Type_Matcher_V3_MetadataMatcher)
    /// Negates matching the provided permission. For instance, if the value of `not_rule` would
    /// match, this permission would not match. Conversely, if the value of `not_rule` would not
    /// match, this permission would match.
    case notRule(Envoy_Config_Rbac_V3_Permission)
    /// The request server from the client's connection request. This is
    /// typically TLS SNI.
    ///
    /// .. attention::
    ...
    /// Please refer to :ref:`this FAQ entry <faq_how_to_setup_sni>` to learn to
    /// setup SNI.
    case requestedServerName(Envoy_Type_Matcher_V3_StringMatcher)

  #if !swift(>=4.1)
    public static func ==(lhs: Envoy_Config_Rbac_V3_Permission.OneOf_Rule, rhs: Envoy_Config_Rbac_V3_Permission.OneOf_Rule) -> Bool {
      switch (lhs, rhs) {
  ...
  public struct Set {
    ...
    public var rules: [Envoy_Config_Rbac_V3_Permission] = []
...
/// [#next-free-field: 12]
public struct Envoy_Config_Rbac_V3_Principal {
  // SwiftProtobuf.Message conformance is added in an extension below. See the
  // `Message` and `Message+*Additions` files in the SwiftProtobuf library for
  // methods supported on all messages.
  ...
  public var andIds: Envoy_Config_Rbac_V3_Principal.Set {
    get {
      if case .andIds(let v)? = _storage._identifier {return v}
      return Envoy_Config_Rbac_V3_Principal.Set()
    }
    set {_uniqueStorage()._identifier = .andIds(newValue)}
  }

  /// A set of identifiers at least one must match in order to define the downstream.
  public var orIds: Envoy_Config_Rbac_V3_Principal.Set {
    get {
  ...
  public var authenticated: Envoy_Config_Rbac_V3_Principal.Authenticated {
    get {
      if case .authenticated(let v)? = _storage._identifier {return v}
      return Envoy_Config_Rbac_V3_Principal.Authenticated()
    }
  ...
  public var sourceIp: Envoy_Config_Core_V3_CidrRange {
    get {
  ...
  public var directRemoteIp: Envoy_Config_Core_V3_CidrRange {
    get {
  ...
  /// proxy protocol, etc.
  public var remoteIp: Envoy_Config_Core_V3_CidrRange {
    get {
  ...
  public var notID: Envoy_Config_Rbac_V3_Principal {
    get {
  ...
  public enum OneOf_Identifier: Equatable {
    /// A set of identifiers that all must match in order to define the downstream.
    case andIds(Envoy_Config_Rbac_V3_Principal.Set)
    /// A set of identifiers at least one must match in order to define the downstream.
    case orIds(Envoy_Config_Rbac_V3_Principal.Set)
    /// When any is set, it matches any downstream.
    case any(Bool)
    /// Authenticated attributes that identify the downstream.
    case authenticated(Envoy_Config_Rbac_V3_Principal.Authenticated)
    /// A CIDR block that describes the downstream IP.
    /// This address will honor proxy protocol, but will not honor XFF.
    case sourceIp(Envoy_Config_Core_V3_CidrRange)
    /// A CIDR block that describes the downstream remote/origin address.
    /// Note: This is always the physical peer even if the
    /// :ref:`remote_ip <envoy_api_field_config.rbac.v3.Principal.remote_ip>` is inferred
    /// from for example the x-forwarder-for header, proxy protocol, etc.
    case directRemoteIp(Envoy_Config_Core_V3_CidrRange)
    /// A CIDR block that describes the downstream remote/origin address.
    /// Note: This may not be the physical peer and could be different from the
    /// :ref:`direct_remote_ip <envoy_api_field_config.rbac.v3.Principal.direct_remote_ip>`.
    /// E.g, if the remote ip is inferred from for example the x-forwarder-for header,
    /// proxy protocol, etc.
    case remoteIp(Envoy_Config_Core_V3_CidrRange)
    /// A header (or pseudo-header such as :path or :method) on the incoming HTTP request. Only
    /// available for HTTP request.
    /// Note: the pseudo-header :path includes the query and fragment string. Use the `url_path`
    /// field if you want to match the URL path without the query and fragment string.
    ...
    /// this principal would not match. Conversely, if the value of `not_id` would not match, this
    /// principal would match.
    case notID(Envoy_Config_Rbac_V3_Principal)

  #if !swift(>=4.1)
    public static func ==(lhs: Envoy_Config_Rbac_V3_Principal.OneOf_Identifier, rhs: Envoy_Config_Rbac_V3_Principal.OneOf_Identifier) -> Bool {
      switch (lhs, rhs) {
  ...
  public struct Set {
    ...
    public var ids: [Envoy_Config_Rbac_V3_Principal] = []
  ...
  public struct Authenticated {
    ...
    /// authenticated.
    public var principalName: Envoy_Type_Matcher_V3_StringMatcher {
      get {return _principalName ?? Envoy_Type_Matcher_V3_StringMatcher()}
    ...
    fileprivate var _principalName: Envoy_Type_Matcher_V3_StringMatcher? = nil
  }
...
fileprivate let _protobuf_package = "envoy.config.rbac.v3"

extension Envoy_Config_Rbac_V3_RBAC: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  ...
  public mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    while let fieldNumber = try decoder.nextFieldNumber() {
      ...
      case 2: try decoder.decodeMapField(fieldType: SwiftProtobuf._ProtobufMessageMap<SwiftProtobuf.ProtobufString,Envoy_Config_Rbac_V3_Policy>.self, value: &self.policies)
      default: break
      }
    }
  }
  ...
  public func traverse<V: SwiftProtobuf.Visitor>(visitor: inout V) throws {
    ...
    if !self.policies.isEmpty {
      try visitor.visitMapField(fieldType: SwiftProtobuf._ProtobufMessageMap<SwiftProtobuf.ProtobufString,Envoy_Config_Rbac_V3_Policy>.self, value: self.policies, fieldNumber: 2)
    }
    try unknownFields.traverse(visitor: &visitor)
  }

  public static func ==(lhs: Envoy_Config_Rbac_V3_RBAC, rhs: Envoy_Config_Rbac_V3_RBAC) -> Bool {
...
extension Envoy_Config_Rbac_V3_RBAC.Action: SwiftProtobuf._ProtoNameProviding {
  public static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
    0: .same(proto: "ALLOW"),
    1: .same(proto: "DENY"),
  ]
...
extension Envoy_Config_Rbac_V3_Policy: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  ...
  public static func ==(lhs: Envoy_Config_Rbac_V3_Policy, rhs: Envoy_Config_Rbac_V3_Policy) -> Bool {
...
extension Envoy_Config_Rbac_V3_Permission: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  ...
  fileprivate class _StorageClass {
    var _rule: Envoy_Config_Rbac_V3_Permission.OneOf_Rule?
  ...
  public mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    ...
    try withExtendedLifetime(_storage) { (_storage: _StorageClass) in
      while let fieldNumber = try decoder.nextFieldNumber() {
        switch fieldNumber {
        case 1:
          var v: Envoy_Config_Rbac_V3_Permission.Set?
        ...
        case 4:
          var v: Envoy_Config_Route_V3_HeaderMatcher?
          if let current = _storage._rule {
            try decoder.handleConflictingOneOf()
        ...
        case 5:
          var v: Envoy_Config_Core_V3_CidrRange?
          if let current = _storage._rule {
            try decoder.handleConflictingOneOf()
        ...
        case 7:
          var v: Envoy_Type_Matcher_V3_MetadataMatcher?
          if let current = _storage._rule {
            try decoder.handleConflictingOneOf()
        ...
        case 8:
          var v: Envoy_Config_Rbac_V3_Permission?
          if let current = _storage._rule {
            try decoder.handleConflictingOneOf()
        ...
        case 10:
          var v: Envoy_Type_Matcher_V3_PathMatcher?
          if let current = _storage._rule {
            try decoder.handleConflictingOneOf()
  ...
  public static func ==(lhs: Envoy_Config_Rbac_V3_Permission, rhs: Envoy_Config_Rbac_V3_Permission) -> Bool {
...
extension Envoy_Config_Rbac_V3_Permission.Set: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  public static let protoMessageName: String = Envoy_Config_Rbac_V3_Permission.protoMessageName + ".Set"
  public static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
    1: .same(proto: "rules"),
  ]
  ...
  public static func ==(lhs: Envoy_Config_Rbac_V3_Permission.Set, rhs: Envoy_Config_Rbac_V3_Permission.Set) -> Bool {
...
extension Envoy_Config_Rbac_V3_Principal: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  ...
  fileprivate class _StorageClass {
    var _identifier: Envoy_Config_Rbac_V3_Principal.OneOf_Identifier?
  ...
  public mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    ...
    try withExtendedLifetime(_storage) { (_storage: _StorageClass) in
      while let fieldNumber = try decoder.nextFieldNumber() {
        switch fieldNumber {
        case 1:
          var v: Envoy_Config_Rbac_V3_Principal.Set?
        ...
        case 4:
          var v: Envoy_Config_Rbac_V3_Principal.Authenticated?
          if let current = _storage._identifier {
            try decoder.handleConflictingOneOf()
        ...
        case 8:
          var v: Envoy_Config_Rbac_V3_Principal?
          if let current = _storage._identifier {
            try decoder.handleConflictingOneOf()
  ...
  public static func ==(lhs: Envoy_Config_Rbac_V3_Principal, rhs: Envoy_Config_Rbac_V3_Principal) -> Bool {
    if lhs._storage !== rhs._storage {
      let storagesAreEqual: Bool = withExtendedLifetime((lhs._storage, rhs._storage)) { (_args: (_StorageClass, _StorageClass)) in
        let _storage = _args.0
        let rhs_storage = _args.1
...
extension Envoy_Config_Rbac_V3_Principal.Set: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  public static let protoMessageName: String = Envoy_Config_Rbac_V3_Principal.protoMessageName + ".Set"
  public static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
    1: .same(proto: "ids"),
  ]
  ...
  public static func ==(lhs: Envoy_Config_Rbac_V3_Principal.Set, rhs: Envoy_Config_Rbac_V3_Principal.Set) -> Bool {
...
extension Envoy_Config_Rbac_V3_Principal.Authenticated: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  public static let protoMessageName: String = Envoy_Config_Rbac_V3_Principal.protoMessageName + ".Authenticated"
  public static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
  ...
  public static func ==(lhs: Envoy_Config_Rbac_V3_Principal.Authenticated, rhs: Envoy_Config_Rbac_V3_Principal.Authenticated) -> Bool {
