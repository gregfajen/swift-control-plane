...
///       principals:
///         - any: true
public struct Envoy_Config_Rbac_V4alpha_RBAC {
  ...
  public var action: Envoy_Config_Rbac_V4alpha_RBAC.Action = .allow

  /// Maps from policy name to policy. A match occurs when at least one policy matches the request.
  public var policies: Dictionary<String,Envoy_Config_Rbac_V4alpha_Policy> = [:]
...
extension Envoy_Config_Rbac_V4alpha_RBAC.Action: CaseIterable {
  // The compiler won't synthesize support with the UNRECOGNIZED case.
  public static var allCases: [Envoy_Config_Rbac_V4alpha_RBAC.Action] = [
    .allow,
    .deny,
    .log,
  ]
...
public struct Envoy_Config_Rbac_V4alpha_Policy {
  // SwiftProtobuf.Message conformance is added in an extension below. See the
  // `Message` and `Message+*Additions` files in the SwiftProtobuf library for
  // methods supported on all messages.
  ...
  public var permissions: [Envoy_Config_Rbac_V4alpha_Permission] = []

  /// Required. The set of principals that are assigned/denied the role based on
  /// “action”. Each principal is matched with OR semantics. To match all
  /// downstreams for this policy, a single Principal with the `any` field set to
  /// true should be used.
  public var principals: [Envoy_Config_Rbac_V4alpha_Principal] = []

  public var expressionSpecifier: Envoy_Config_Rbac_V4alpha_Policy.OneOf_ExpressionSpecifier? = nil
  ...
  public var condition: Google_Api_Expr_V1alpha1_Expr {
    get {
      if case .condition(let v)? = expressionSpecifier {return v}
      return Google_Api_Expr_V1alpha1_Expr()
    }
    set {expressionSpecifier = .condition(newValue)}
  }

  /// [#not-implemented-hide:]
  /// An optional symbolic expression that has been successfully type checked.
  /// Only be used when condition is not used.
  public var checkedCondition: Google_Api_Expr_V1alpha1_CheckedExpr {
    get {
      if case .checkedCondition(let v)? = expressionSpecifier {return v}
      return Google_Api_Expr_V1alpha1_CheckedExpr()
    }
    set {expressionSpecifier = .checkedCondition(newValue)}
  }

  public var unknownFields = SwiftProtobuf.UnknownStorage()

  public enum OneOf_ExpressionSpecifier: Equatable {
    /// An optional symbolic expression specifying an access control
    /// :ref:`condition <arch_overview_condition>`. The condition is combined
    /// with the permissions and the principals as a clause with AND semantics.
    /// Only be used when checked_condition is not used.
    case condition(Google_Api_Expr_V1alpha1_Expr)
    /// [#not-implemented-hide:]
    /// An optional symbolic expression that has been successfully type checked.
    /// Only be used when condition is not used.
    case checkedCondition(Google_Api_Expr_V1alpha1_CheckedExpr)

  #if !swift(>=4.1)
    public static func ==(lhs: Envoy_Config_Rbac_V4alpha_Policy.OneOf_ExpressionSpecifier, rhs: Envoy_Config_Rbac_V4alpha_Policy.OneOf_ExpressionSpecifier) -> Bool {
      ...
      case (.checkedCondition, .checkedCondition): return {
        guard case .checkedCondition(let l) = lhs, case .checkedCondition(let r) = rhs else { preconditionFailure() }
        return l == r
      }()
...
/// [#next-free-field: 11]
public struct Envoy_Config_Rbac_V4alpha_Permission {
  // SwiftProtobuf.Message conformance is added in an extension below. See the
  // `Message` and `Message+*Additions` files in the SwiftProtobuf library for
  // methods supported on all messages.
  ...
  public var andRules: Envoy_Config_Rbac_V4alpha_Permission.Set {
    get {
      if case .andRules(let v)? = _storage._rule {return v}
      return Envoy_Config_Rbac_V4alpha_Permission.Set()
    }
    set {_uniqueStorage()._rule = .andRules(newValue)}
  }

  /// A set of rules where at least one must match in order to define the action.
  public var orRules: Envoy_Config_Rbac_V4alpha_Permission.Set {
    get {
  ...
  public var header: Envoy_Config_Route_V4alpha_HeaderMatcher {
    get {
      ...
      return Envoy_Config_Route_V4alpha_HeaderMatcher()
    }
    set {_uniqueStorage()._rule = .header(newValue)}
  }

  /// A URL path on the incoming HTTP request. Only available for HTTP.
  public var urlPath: Envoy_Type_Matcher_V4alpha_PathMatcher {
    get {
      if case .urlPath(let v)? = _storage._rule {return v}
      return Envoy_Type_Matcher_V4alpha_PathMatcher()
    }
    set {_uniqueStorage()._rule = .urlPath(newValue)}
  }

  /// A CIDR block that describes the destination IP.
  public var destinationIp: Envoy_Config_Core_V4alpha_CidrRange {
    get {
      if case .destinationIp(let v)? = _storage._rule {return v}
      return Envoy_Config_Core_V4alpha_CidrRange()
    }
  ...
  public var metadata: Envoy_Type_Matcher_V4alpha_MetadataMatcher {
    get {
      if case .metadata(let v)? = _storage._rule {return v}
      return Envoy_Type_Matcher_V4alpha_MetadataMatcher()
    }
  ...
  public var notRule: Envoy_Config_Rbac_V4alpha_Permission {
    get {
      if case .notRule(let v)? = _storage._rule {return v}
      return Envoy_Config_Rbac_V4alpha_Permission()
    }
  ...
  /// setup SNI.
  public var requestedServerName: Envoy_Type_Matcher_V4alpha_StringMatcher {
    get {
  ...
  public enum OneOf_Rule: Equatable {
    /// A set of rules that all must match in order to define the action.
    case andRules(Envoy_Config_Rbac_V4alpha_Permission.Set)
    /// A set of rules where at least one must match in order to define the action.
    case orRules(Envoy_Config_Rbac_V4alpha_Permission.Set)
    /// When any is set, it matches any action.
    case any(Bool)
    /// A header (or pseudo-header such as :path or :method) on the incoming HTTP request. Only
    /// available for HTTP request.
    /// Note: the pseudo-header :path includes the query and fragment string. Use the `url_path`
    /// field if you want to match the URL path without the query and fragment string.
    case header(Envoy_Config_Route_V4alpha_HeaderMatcher)
    /// A URL path on the incoming HTTP request. Only available for HTTP.
    case urlPath(Envoy_Type_Matcher_V4alpha_PathMatcher)
    /// A CIDR block that describes the destination IP.
    case destinationIp(Envoy_Config_Core_V4alpha_CidrRange)
    /// A port number that describes the destination port connecting to.
    case destinationPort(UInt32)
    /// Metadata that describes additional information about the action.
    case metadata(Envoy_Type_Matcher_V4alpha_MetadataMatcher)
    /// Negates matching the provided permission. For instance, if the value of
    /// `not_rule` would match, this permission would not match. Conversely, if
    /// the value of `not_rule` would not match, this permission would match.
    case notRule(Envoy_Config_Rbac_V4alpha_Permission)
    /// The request server from the client's connection request. This is
    /// typically TLS SNI.
    ///
    /// .. attention::
    ...
    /// Please refer to :ref:`this FAQ entry <faq_how_to_setup_sni>` to learn to
    /// setup SNI.
    case requestedServerName(Envoy_Type_Matcher_V4alpha_StringMatcher)

  #if !swift(>=4.1)
    public static func ==(lhs: Envoy_Config_Rbac_V4alpha_Permission.OneOf_Rule, rhs: Envoy_Config_Rbac_V4alpha_Permission.OneOf_Rule) -> Bool {
      // The use of inline closures is to circumvent an issue where the compiler
      // allocates stack space for every case branch when no optimizations are
      // enabled. https://github.com/apple/swift-protobuf/issues/1034
      switch (lhs, rhs) {
  ...
  public struct Set {
    ...
    public var rules: [Envoy_Config_Rbac_V4alpha_Permission] = []
...
/// subject.
/// [#next-free-field: 12]
public struct Envoy_Config_Rbac_V4alpha_Principal {
  // SwiftProtobuf.Message conformance is added in an extension below. See the
  // `Message` and `Message+*Additions` files in the SwiftProtobuf library for
  // methods supported on all messages.
  ...
  /// downstream.
  public var andIds: Envoy_Config_Rbac_V4alpha_Principal.Set {
    get {
      if case .andIds(let v)? = _storage._identifier {return v}
      return Envoy_Config_Rbac_V4alpha_Principal.Set()
    }
  ...
  /// downstream.
  public var orIds: Envoy_Config_Rbac_V4alpha_Principal.Set {
    get {
  ...
  public var authenticated: Envoy_Config_Rbac_V4alpha_Principal.Authenticated {
    get {
      if case .authenticated(let v)? = _storage._identifier {return v}
      return Envoy_Config_Rbac_V4alpha_Principal.Authenticated()
    }
  ...
  /// etc.
  public var directRemoteIp: Envoy_Config_Core_V4alpha_CidrRange {
    get {
  ...
  /// protocol, etc.
  public var remoteIp: Envoy_Config_Core_V4alpha_CidrRange {
    get {
  ...
  public var notID: Envoy_Config_Rbac_V4alpha_Principal {
    get {
      if case .notID(let v)? = _storage._identifier {return v}
      return Envoy_Config_Rbac_V4alpha_Principal()
    }
  ...
  public enum OneOf_Identifier: Equatable {
    /// A set of identifiers that all must match in order to define the
    /// downstream.
    case andIds(Envoy_Config_Rbac_V4alpha_Principal.Set)
    /// A set of identifiers at least one must match in order to define the
    /// downstream.
    case orIds(Envoy_Config_Rbac_V4alpha_Principal.Set)
    /// When any is set, it matches any downstream.
    case any(Bool)
    /// Authenticated attributes that identify the downstream.
    case authenticated(Envoy_Config_Rbac_V4alpha_Principal.Authenticated)
    /// A CIDR block that describes the downstream remote/origin address.
    /// Note: This is always the physical peer even if the
    /// :ref:`remote_ip <envoy_api_field_config.rbac.v4alpha.Principal.remote_ip>` is
    /// inferred from for example the x-forwarder-for header, proxy protocol,
    /// etc.
    case directRemoteIp(Envoy_Config_Core_V4alpha_CidrRange)
    /// A CIDR block that describes the downstream remote/origin address.
    /// Note: This may not be the physical peer and could be different from the
    /// :ref:`direct_remote_ip
    /// <envoy_api_field_config.rbac.v4alpha.Principal.direct_remote_ip>`. E.g, if the
    /// remote ip is inferred from for example the x-forwarder-for header, proxy
    /// protocol, etc.
    case remoteIp(Envoy_Config_Core_V4alpha_CidrRange)
    /// A header (or pseudo-header such as :path or :method) on the incoming HTTP
    /// request. Only available for HTTP request. Note: the pseudo-header :path
    /// includes the query and fragment string. Use the `url_path` field if you
    /// want to match the URL path without the query and fragment string.
    ...
    /// `not_id` would match, this principal would not match. Conversely, if the
    /// value of `not_id` would not match, this principal would match.
    case notID(Envoy_Config_Rbac_V4alpha_Principal)

  #if !swift(>=4.1)
    public static func ==(lhs: Envoy_Config_Rbac_V4alpha_Principal.OneOf_Identifier, rhs: Envoy_Config_Rbac_V4alpha_Principal.OneOf_Identifier) -> Bool {
      // The use of inline closures is to circumvent an issue where the compiler
      // allocates stack space for every case branch when no optimizations are
      // enabled. https://github.com/apple/swift-protobuf/issues/1034
      switch (lhs, rhs) {
  ...
  public struct Set {
    ...
    public var ids: [Envoy_Config_Rbac_V4alpha_Principal] = []
  ...
  public struct Authenticated {
    ...
    public var principalName: Envoy_Type_Matcher_V4alpha_StringMatcher {
      get {return _principalName ?? Envoy_Type_Matcher_V4alpha_StringMatcher()}
    ...
    fileprivate var _principalName: Envoy_Type_Matcher_V4alpha_StringMatcher? = nil
  }
...
fileprivate let _protobuf_package = "envoy.config.rbac.v4alpha"

extension Envoy_Config_Rbac_V4alpha_RBAC: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  ...
  public mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    while let fieldNumber = try decoder.nextFieldNumber() {
      ...
      case 2: try { try decoder.decodeMapField(fieldType: SwiftProtobuf._ProtobufMessageMap<SwiftProtobuf.ProtobufString,Envoy_Config_Rbac_V4alpha_Policy>.self, value: &self.policies) }()
      default: break
      }
    }
  }
  ...
  public func traverse<V: SwiftProtobuf.Visitor>(visitor: inout V) throws {
    ...
    if !self.policies.isEmpty {
      try visitor.visitMapField(fieldType: SwiftProtobuf._ProtobufMessageMap<SwiftProtobuf.ProtobufString,Envoy_Config_Rbac_V4alpha_Policy>.self, value: self.policies, fieldNumber: 2)
    }
    try unknownFields.traverse(visitor: &visitor)
  }

  public static func ==(lhs: Envoy_Config_Rbac_V4alpha_RBAC, rhs: Envoy_Config_Rbac_V4alpha_RBAC) -> Bool {
...
extension Envoy_Config_Rbac_V4alpha_RBAC.Action: SwiftProtobuf._ProtoNameProviding {
  public static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
    0: .same(proto: "ALLOW"),
    1: .same(proto: "DENY"),
    2: .same(proto: "LOG"),
...
extension Envoy_Config_Rbac_V4alpha_Policy: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  ...
  public static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
    ...
    4: .standard(proto: "checked_condition"),
  ]
  ...
  public mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    while let fieldNumber = try decoder.nextFieldNumber() {
      ...
      case 3: try {
        ...
        if let current = self.expressionSpecifier {
          try decoder.handleConflictingOneOf()
          if case .condition(let m) = current {v = m}
        }
        try decoder.decodeSingularMessageField(value: &v)
        if let v = v {self.expressionSpecifier = .condition(v)}
      }()
      case 4: try {
        var v: Google_Api_Expr_V1alpha1_CheckedExpr?
        if let current = self.expressionSpecifier {
          try decoder.handleConflictingOneOf()
          if case .checkedCondition(let m) = current {v = m}
        }
        try decoder.decodeSingularMessageField(value: &v)
        if let v = v {self.expressionSpecifier = .checkedCondition(v)}
      }()
      default: break
      }
    }
  ...
  public func traverse<V: SwiftProtobuf.Visitor>(visitor: inout V) throws {
    ...
    // allocates stack space for every case branch when no optimizations are
    // enabled. https://github.com/apple/swift-protobuf/issues/1034
    switch self.expressionSpecifier {
    case .condition?: try {
      guard case .condition(let v)? = self.expressionSpecifier else { preconditionFailure() }
      try visitor.visitSingularMessageField(value: v, fieldNumber: 3)
    }()
    case .checkedCondition?: try {
      guard case .checkedCondition(let v)? = self.expressionSpecifier else { preconditionFailure() }
  ...
  public static func ==(lhs: Envoy_Config_Rbac_V4alpha_Policy, rhs: Envoy_Config_Rbac_V4alpha_Policy) -> Bool {
    if lhs.permissions != rhs.permissions {return false}
    if lhs.principals != rhs.principals {return false}
    if lhs.expressionSpecifier != rhs.expressionSpecifier {return false}
    if lhs.unknownFields != rhs.unknownFields {return false}
    return true
  }
}

extension Envoy_Config_Rbac_V4alpha_Permission: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  ...
  fileprivate class _StorageClass {
    var _rule: Envoy_Config_Rbac_V4alpha_Permission.OneOf_Rule?
  ...
  public mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    ...
    try withExtendedLifetime(_storage) { (_storage: _StorageClass) in
      while let fieldNumber = try decoder.nextFieldNumber() {
        ...
        switch fieldNumber {
        case 1: try {
          var v: Envoy_Config_Rbac_V4alpha_Permission.Set?
        ...
        case 4: try {
          var v: Envoy_Config_Route_V4alpha_HeaderMatcher?
          if let current = _storage._rule {
            try decoder.handleConflictingOneOf()
        ...
        case 5: try {
          var v: Envoy_Config_Core_V4alpha_CidrRange?
          if let current = _storage._rule {
            try decoder.handleConflictingOneOf()
        ...
        case 7: try {
          var v: Envoy_Type_Matcher_V4alpha_MetadataMatcher?
          if let current = _storage._rule {
            try decoder.handleConflictingOneOf()
        ...
        case 8: try {
          var v: Envoy_Config_Rbac_V4alpha_Permission?
          if let current = _storage._rule {
            try decoder.handleConflictingOneOf()
        ...
        case 10: try {
          var v: Envoy_Type_Matcher_V4alpha_PathMatcher?
          if let current = _storage._rule {
            try decoder.handleConflictingOneOf()
  ...
  public static func ==(lhs: Envoy_Config_Rbac_V4alpha_Permission, rhs: Envoy_Config_Rbac_V4alpha_Permission) -> Bool {
...
extension Envoy_Config_Rbac_V4alpha_Permission.Set: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  public static let protoMessageName: String = Envoy_Config_Rbac_V4alpha_Permission.protoMessageName + ".Set"
  public static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
    1: .same(proto: "rules"),
  ]
  ...
  public static func ==(lhs: Envoy_Config_Rbac_V4alpha_Permission.Set, rhs: Envoy_Config_Rbac_V4alpha_Permission.Set) -> Bool {
...
extension Envoy_Config_Rbac_V4alpha_Principal: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  ...
  fileprivate class _StorageClass {
    var _identifier: Envoy_Config_Rbac_V4alpha_Principal.OneOf_Identifier?
  ...
  public mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    ...
    try withExtendedLifetime(_storage) { (_storage: _StorageClass) in
      while let fieldNumber = try decoder.nextFieldNumber() {
        ...
        switch fieldNumber {
        case 1: try {
          var v: Envoy_Config_Rbac_V4alpha_Principal.Set?
        ...
        case 4: try {
          var v: Envoy_Config_Rbac_V4alpha_Principal.Authenticated?
          if let current = _storage._identifier {
            try decoder.handleConflictingOneOf()
        ...
        case 8: try {
          var v: Envoy_Config_Rbac_V4alpha_Principal?
          if let current = _storage._identifier {
            try decoder.handleConflictingOneOf()
  ...
  public static func ==(lhs: Envoy_Config_Rbac_V4alpha_Principal, rhs: Envoy_Config_Rbac_V4alpha_Principal) -> Bool {
    if lhs._storage !== rhs._storage {
      let storagesAreEqual: Bool = withExtendedLifetime((lhs._storage, rhs._storage)) { (_args: (_StorageClass, _StorageClass)) in
        let _storage = _args.0
        let rhs_storage = _args.1
...
extension Envoy_Config_Rbac_V4alpha_Principal.Set: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  public static let protoMessageName: String = Envoy_Config_Rbac_V4alpha_Principal.protoMessageName + ".Set"
  public static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
    1: .same(proto: "ids"),
  ]
  ...
  public static func ==(lhs: Envoy_Config_Rbac_V4alpha_Principal.Set, rhs: Envoy_Config_Rbac_V4alpha_Principal.Set) -> Bool {
...
extension Envoy_Config_Rbac_V4alpha_Principal.Authenticated: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  public static let protoMessageName: String = Envoy_Config_Rbac_V4alpha_Principal.protoMessageName + ".Authenticated"
  public static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
  ...
  public static func ==(lhs: Envoy_Config_Rbac_V4alpha_Principal.Authenticated, rhs: Envoy_Config_Rbac_V4alpha_Principal.Authenticated) -> Bool {
