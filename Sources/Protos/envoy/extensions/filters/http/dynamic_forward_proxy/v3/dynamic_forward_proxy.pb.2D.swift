...
extension Envoy_Extensions_Filters_Http_DynamicForwardProxy_V3_PerRouteConfig: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  ...
  public func traverse<V: SwiftProtobuf.Visitor>(visitor: inout V) throws {
    ...
    case .hostRewriteLiteral?: try {
      guard case .hostRewriteLiteral(let v)? = self.hostRewriteSpecifier else { preconditionFailure() }
      try visitor.visitSingularStringField(value: v, fieldNumber: 1)
    }()
    case .hostRewriteHeader?: try {
      guard case .hostRewriteHeader(let v)? = self.hostRewriteSpecifier else { preconditionFailure() }
